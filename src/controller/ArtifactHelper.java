...
public class ArtifactHelper {
	...
	public List<Artifact> showAllArtifacts() {
		EntityManager em = emfactory.createEntityManager();
		List<Artifact> allArtifacts = em.createQuery("SELECT i FROM Artifact i").getResultList();
		return allArtifacts;
	}
	
	public List<Artifact> searchForArtifactsById(int presID) {
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<Artifact> typedQuery = em.createQuery("select ar from Artifact ar where ar.president.id = :selectedNum", Artifact.class);
		typedQuery.setParameter("selectedNum", presID);

		List<Artifact> foundArtifacts = typedQuery.getResultList();
		em.close();
